...
static void set_layer_status(struct zmk_widget_screen *widget, struct layer_status_state state) {
    ...
    widget->state.os_layer_active = state.os_layer_active;
...
static struct layer_status_state layer_status_get_state(const zmk_event_t *eh) {
    int index = zmk_keymap_highest_layer_active();
    bool os_active = false;
#if CONFIG_NICE_VIEW_GEM_OS_LAYER >= 0
    os_active = zmk_keymap_layer_active(zmk_keymap_layer_index_to_id(CONFIG_NICE_VIEW_GEM_OS_LAYER));
    if (index == CONFIG_NICE_VIEW_GEM_OS_LAYER) {
        // OS marker layer is transparent; show the layer underneath instead
        for (int i = index - 1; i >= 0; i--) {
            if (zmk_keymap_layer_active(zmk_keymap_layer_index_to_id(i))) {
                index = i;
                break;
            }
        }
